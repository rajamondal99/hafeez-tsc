...
class SaleOrderExtend(models.Model):
    ...
    def _amount_all(self):
        for order in self:
            ...
            amount_untaxed = amount_untaxed - order.discount_amount
            order.update({
                'amount_untaxed': order.pricelist_id.currency_id.round(amount_untaxed),
                'amount_tax': order.pricelist_id.currency_id.round(amount_tax),
                'amount_total': amount_untaxed + amount_tax,
		'amount_total_undiscounted':amount_undiscounted,
 ...
 # For displaying a single record. Remove if you want to display tree view
    ...
    def create_purchase_request(self,sale_stock_loc_ids):
        ...
        vals = {
                ...
                'description':"A salesperson was about selling the following listed items with zero stock level. Please request for the items to be purchased from the manager. The sales order reference is: " + self.name ,
                'line_ids':lines
            }
    ...
    amount_total_undiscounted = fields.Float(string="Subtotal",store=True)
