    ...
    def compute_OP(self, modeFull=True):
        ...
        # New in openERP...  Create on create_events of synchronize function
            ...
            if self.OE.status:
                ...
                if not modeFull:
                    self.OP = Delete('GG', 'Deleted from the System, need to delete it from Gmail if already created')
                else:
                    self.OP = NothingToDo("", "Already Deleted in gmail and unlinked in the System")
...
all links between GoogleID and System Google Internal ID', copy=False)
    }
...
class calendar_event(osv.Model):
    ...
    _columns = {
        'oe_update_date': fields.datetime('System Update Date'),
    }
...
class calendar_attendee(osv.Model):
    ...
    _columns = {
        ...
        'oe_synchro_date': fields.datetime('System Synchro Date'),
    }
